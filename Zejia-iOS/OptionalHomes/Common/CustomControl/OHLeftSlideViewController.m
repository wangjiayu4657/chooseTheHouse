...
@interface OHLeftSlideViewController ()<UIGestureRecognizerDelegate>
{
...
@end
@implementation OHLeftSlideViewController
- (void)viewDidLoad {
    [super viewDidLoad];
                     ...
                     andMainView:(UIViewController *)mainVC
{
    if(self = [super init]){
        ...
        _coverView.frame = self.mainVC.view.bounds;
        ...
        self.leftTableview.frame = CGRectMake(0, 0, kScreenWidth - kMainPageDistance, kScreenHeight- kBottomHeight);
        //设置左侧tableview的初始位置和缩放系数
        self.leftTableview.transform = CGAffineTransformMakeScale(kLeftScale, kLeftScale);
        self.leftTableview.center = CGPointMake(kLeftCenterX, (kScreenHeight-kBottomHeight) * 0.5);
...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance)))
    {
        ...
        self.leftTableview.center = CGPointMake(leftTabCenterX, (kScreenHeight-kBottomHeight) * 0.5);
...
- (void)openLeftView;
{
    ...
    self.leftTableview.center = CGPointMake((kScreenWidth - kMainPageDistance) * 0.5, (kScreenHeight-kBottomHeight) * 0.5);
